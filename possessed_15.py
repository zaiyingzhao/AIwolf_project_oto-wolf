...
class PossessedBehavior(villager_15.VillagerBehavior):
    ...
    def talk(self):
        ...
        if self.base_info["day"] == 1:
            if self.talk_turn == 1:
                CO_SEER = cb.comingout(self.base_info["agentIdx"], RoleType.SEER.value)
                return CO_SEER
            if self.talk_turn == 2:
                if len(self.COs) == 3:
                    vote_cand = (
                    ...
                    self.ts_blackness.append(target)
                    self.blackness.add(target)
                    fake_divine_res = cb.divined(target, RoleType.WEREWOLF.value)
        ...
        if self.base_info["day"] >= 2:
            if len(self.alive) > 7:
                if self.talk_turn == 1:
                    if self.ts_blackness[-1] not in self.alive:
                        result_all_white = set()
                        non_me_divineders = self.divineders - {
                            int(self.base_info["agentIdx"])
                        }
                        ...
                        if result_all_white:
                            ...
                            self.ts_blackness.append(target)
                            self.blackness.add(target)
                            self.result_seer["black"] = self.blackness
                            fake_divine_res = cb.divined(
                                target, RoleType.WEREWOLF.value
                            )
                            return fake_divine_res
                        else:
                            stupid = get_stupid(self.vote_list, self.ts_blackness[-1])
                        ...
                        noMAX_vote_list = [
                            s for s in self.vote_list if s != self.exed_players[-1]
                        ]
                        noME_noMAX_vote_list = [
                            s
                            for s in noMAX_vote_list
                            if s != int(self.base_info["agentIdx"])
                        ]
                        ...
                        if cand:
                            ...
                            target = random.choice(
                                list(self.alive - {int(self.base_info["agentIdx"])})
                            )
                        self.whiteness.add(target)
                        self.result_seer["white"] = self.whiteness
                        fake_divine_res = cb.divined(target, RoleType.HUMAN.value)
                        return fake_divine_res
                else:
                    return super().talk()
            else:
                ...
                if self.talk_turn == 1:
                    return cb.comingout(
                        self.base_info["agentIdx"], RoleType.POSSESSED.value
                    )
                if self.talk_turn >= 2:
    ...
    def vote(self):
        if self.SP == 0:
            most_voted = util.max_frequent_2(
                self.talk_vote_list, self.decide_vote_cand(), 1
            )
            return most_voted
        else:
    ...
    def decide_vote_cand(self):
        if (
            max_frequent(self.talk_vote_list)
            & self.alive
            & (self.COs | self.divineders | set(self.COm))
            - {int(self.base_info["agentIdx"])}
        ):
            target = (
                max_frequent(self.talk_vote_list)
            ...
            return target
        if self.blackness & self.alive:
            target = self.blackness & self.alive
            return target
        black_cand = set()
        for key in (
            self.divineders - self.fake_divineders - {int(self.base_info["agentIdx"])}
        ):
        ...
        if black_cand:
            ...
            return target
        if (
            (self.likely_white_set & self.alive)
            - self.whiteness
            - self.divineders
            - self.COm
        ):
        ...
        target = self.alive - self.divineders - self.whiteness - set(self.COm[:1])
        if target:
            return target
        if self.divineders - {int(self.base_info["agentIdx"])} & self.alive:
            target = self.divineders - {int(self.base_info["agentIdx"])} & self.alive
            return target
        if self.whiteness & self.alive:
            target = self.whiteness & self.alive
            return target
