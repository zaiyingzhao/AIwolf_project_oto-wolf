...
class WerewolfBehavior(villager_15.VillagerBehavior):
    ...
    def initialize(self, base_info, diff_data, game_setting):
        ...
        judge_thresh = random.random()

        if judge_thresh <= 0.1:
            self.role_decision = 0
        elif judge_thresh > 0.1 and judge_thresh <= 0.5:
    ...
    def talk(self):
        ...
        if self.role_decision == 0:
            if self.base_info["day"] == 1:
                if self.stealth == 0:
                    if self.talk_turn == 1:
                        CO_SEER = cb.comingout(
                            self.base_info["agentIdx"], util.RoleType.SEER.value
                        )
                        return CO_SEER
                    elif self.talk_turn == 2:
                        if self.COs - {int(self.base_info["agentIdx"])}:
                            self.shonichi_target = random.choice(
                                list(self.COs - {int(self.base_info["agentIdx"])})
                            )
                            self.blackness.add(self.shonichi_target)
                            return cb.divined(
                                self.shonichi_target, util.RoleType.WEREWOLF.value
                            )
                        else:
                            self.shonichi_target = random.choice(
                                list(
                    ...
                    if judger <= 0.7 and self.SPcounter <= 2:
                        if self.talk_turn == 1:
                            if (
                                ...
                                self.whiteness.add(target)
                                return cb.divined(target, util.RoleType.HUMAN.value)
                            elif self.alive - self.blackness - self.whiteness:
                                target = random.choice(
                                    list(self.alive - self.blackness - self.whiteness)
                                )
                                self.whiteness.add(target)
                                return cb.divined(target, util.RoleType.HUMAN.value)
                            elif self.alive & self.whiteness:
                                target = random.choice(
                                    list(self.alive & self.whiteness)
                                )
                                self.whiteness.add(target)
                                return cb.divined(target, util.RoleType.HUMAN.value)
                            else:
                                target = random.choice(
                                    list(self.alive & self.blackness)
                                )
                                self.blackness.add(target)
                                return cb.divined(target, util.RoleType.WEREWOLF.value)
                        else:
                            target = self.vote()
                        ...
                        if self.talk_turn == 1:
                            if self.alive & self.wolfs - {
                                ...
                                target = random.choice(
                                    list(
                                        self.alive
                                        & self.wolfs - {int(self.base_info["agentIdx"])}
                                    )
                                )
                                self.whiteness.add(target)
                                return cb.divined(target, util.RoleType.HUMAN.value)
        ...
        if self.role_decision == 2:
            if self.base_info["day"] == 1:
                if self.talk_turn == 1:
                    if self.stealth == 0:
                        CO_MEDIUM = cb.comingout(
                            self.base_info["agentIdx"], util.RoleType.MEDIUM.value
                        )
                        return CO_MEDIUM
                    else:
                        return super().talk()
                    ...
                    if self.talk_turn == 1:
                        if self.exed_players[-1] in set(self.COm):
                            ...
                            if self.first_identify == 1:
                                if len(self.COm) >= 2:
                                    IDENTIFIED_WHITE = cb.identified(
                                        self.exed_players[-1], util.RoleType.HUMAN.value
                                    )
                                    self.first_identify = 1
                            ...
                            if self.exed_players[-1] in black_cand:
                                IDENTIFIED_BLACK = cb.identified(
                                    self.exed_players[-1], util.RoleType.WEREWOLF.value
                                )
                                return IDENTIFIED_BLACK
                            else:
        ...
        if self.base_info["day"] == 1:
            ...
            if len(self.alive) <= 7:
                if len(self.COp) != 0:
                    ...
                    if self.decide_vote_cand() - self.whiteness:
                        cands = self.decide_vote_cand() - self.whiteness
                        most_voted = util.max_frequent_2(self.talk_vote_list, cands, 1)
                        return most_voted
                    else:
                        cands = self.decide_vote_cand()
            ...
            if self.base_info["day"] == 0:
                ...
                if self.whisper_turn == 2:
                    if len(self.W_COs) >= 2 or len(self.W_COm) >= 1:
                        self.stealth = 1
                        return cb.comingout(
                            self.base_info["agentIdx"], util.RoleType.VILLAGER.value
                        )
                    else:
                        return cb.skip()
    ...
    def attack(self):
        ...
        if self.role_decision == 1:
            if self.base_info["day"] == 1:
                likely_true_divineders = (
                    self.alive
                    & self.divineders
                    - self.fake_divineders
                    - self.wolfs
                    - self.likely_fake_divineders
                )
        ...
        if self.role_decision == 2:
            ...
            white_cand = set()
            may_seer = (
                self.divineders - self.fake_divineders - self.likely_fake_divineders
            )
            for i in may_seer:
    ...
    def pp_judger(self):
        if (
            len(self.divineders & self.alive - self.wolfs) >= 2
            or len(set(self.likely_fake_divineders.keys()) & set(self.alive))
            - len(set(self.wolfs))
            >= 1
        ):
