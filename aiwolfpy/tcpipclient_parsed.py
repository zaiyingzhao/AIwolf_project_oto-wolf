...
def connect_parse(agent):
    ...
    parser.add_argument('-n', type=str, action='store', dest='name', default="otowolf")
            ...
            while buffer_flg == 1:
                ...
                if request == "NAME":
                    ...
                    sock.send(
                        (
                            json.dumps(
                                {"agentIdx": int(agent.vote())}, separators=(",", ":")
                            )
                            + "\n"
                        ).encode("utf-8")
                    )
                    ...
                    sock.send(
                        (
                            json.dumps(
                                {"agentIdx": int(agent.attack())}, separators=(",", ":")
                            )
                            + "\n"
                        ).encode("utf-8")
                    )
                    ...
                    sock.send(
                        (
                            json.dumps(
                                {"agentIdx": int(agent.guard())}, separators=(",", ":")
                            )
                            + "\n"
                        ).encode("utf-8")
                    )
                    ...
                    sock.send(
                        (
                            json.dumps(
                                {"agentIdx": int(agent.divine())}, separators=(",", ":")
                            )
                            + "\n"
                        ).encode("utf-8")
                    )
