...
class GameInfoParser(object):
    ...
    def update(self, game_info, talk_history, whisper_history, request):
        ...
        if (
            request == "WHISPER"
            or request == "DAILY_FINISH"
            and len(whisper_history) > 0
        ):
            ...
            if self.night_info == 0:
                ...
                for v in game_info["voteList"]:
                    ...
                    self.pd_dict["text"].append(
                        "VOTE Agent[" + "{0:02d}".format(v["target"]) + "]"
                    )
            ...
            if game_info["guardedAgent"] != -1:
                ...
                self.pd_dict["text"].append(
                    "GUARDED Agent[" + "{0:02d}".format(game_info["guardedAgent"]) + "]"
                )
            ...
            for v in game_info["attackVoteList"]:
                ...
                self.pd_dict["agent"].append(v["target"])
                self.pd_dict["text"].append(
                    "ATTACK Agent[" + "{0:02d}".format(v["target"]) + "]"
                )
            ...
            if game_info["attackedAgent"] != -1:
                ...
                self.pd_dict["text"].append(
                    "ATTACK Agent[" + "{0:02d}".format(game_info["attackedAgent"]) + "]"
                )
        ...
        # VOTE/EXECUTE before action
            ...
            request in ["DIVINE", "GUARD", "ATTACK", "WHISPER"] and self.night_info == 0
        ):
            # VOTE
