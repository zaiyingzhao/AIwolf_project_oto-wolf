...
class SeerBehavior(villager_15.VillagerBehavior):
            ...
            for i in range(diff_data.shape[0]):
                if diff_data["type"][i] == "divine":
                    ...
                    if int(content[1][6:8]) in range(1, 16):
                        if content[2] == util.RoleType.HUMAN.value:
    ...
    def talk(self):
        ...
        if self.base_info["day"] == 1 and self.talk_turn == 1:
            return cb.comingout(self.base_info["agentIdx"], util.RoleType.SEER.value)
        elif self.result_seer_new:
            who, result = self.result_seer_new.pop(0)
            result = (
                util.RoleType.WEREWOLF.value if result else util.RoleType.HUMAN.value
            )
    ...
    def vote_cand(self):
        ...
        may_black -= (
            self.result_seer["white"]
            | self.result_seer["black"]
